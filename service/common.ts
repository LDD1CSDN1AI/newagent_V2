...
import { API_PREFIX2 } from '@/config'
...
export const fetchUserProfile: Fetcher<UserProfileOriginResponse, { url: string; params: Record<string, any> }> = ({ url, params }) => {
  return get<UserProfileOriginResponse>(GlobalUrl.defaultUrlIp + "/console/api" + url, params, { needAllResponseContent: true })
}
...
export const fetchLanggeniusVersion: Fetcher<LangGeniusVersionResponse, { url: string; params: Record<string, any> }> = ({ url, params }) => {
  return get<LangGeniusVersionResponse>(GlobalUrl.defaultUrlIp + "/console/api" + url, { params })
}
...
export const fetchCurrentWorkspace: Fetcher<ICurrentWorkspace, { url: string; params: Record<string, any> }> = ({ url, params }) => {
  return get<ICurrentWorkspace>(GlobalUrl.defaultUrlIp + "/console/api" + url, { params })
}
...
export const fetchInstallPlugin: Fetcher<PluginProvider[] | null, string> = (url) => {
  return get<PluginProvider[] | null>(url)
}

export const fetchDcoosList: Fetcher<any[] | null, string> = (url) => {
  return get<any[] | null>(url)
}

export const fetchLlmList: Fetcher<any[] | null, string> = (url) => {
  return get<any[] | null>(url)
}

export const downDcoosApi: Fetcher<any> = (url: string) => {
  return get<any>(url)
}

export const downshuzhiApi: Fetcher<any, { body: any }> = ({ body }) => {
  return post<any>(`http://127.0.0.1:5001/intf-restful-service/deploy/modelRegisterApi`, { body })
}

export const delCommon: Fetcher<any | string> = (url: string) => {
  return get<any>(url)
}
...
export const addDcoosApi: Fetcher<any, { url: string; body: any }> = ({ url, body }) => {
  return post<any>(url, { body })
}

export const fetchModelProviders: Fetcher<{ data: ModelProvider[] }, string> = (url) => {
  return get<{ data: ModelProvider[] }>(GlobalUrl.defaultUrlIp + "/console/api" + url)
}
...
export const getTenants: Fetcher<{ url: string }, string> = (url) => {
  return get<{ url: string }>(url)
}
...
export const fetchSupportRetrievalMethods: Fetcher<RetrievalMethodsRes, string> = (url) => {
  return get<RetrievalMethodsRes>(GlobalUrl.defaultUrlIp + "/console/api" + url)
}

export const getSystemFeatures = () => {
  return get<SystemFeatures>(GlobalUrl.defaultUrlIp + "/console/api" + '/system-features')
}
...
export const meunClickLog: Fetcher<any, { body: any }> = ({ body }) => {
  return post<any>(`${API_PREFIX2}/interface/api/statistics/menuClickLog/addInterface`, { body })
}
export const getTenantMember: Fetcher<{ url: string }, string> = (url) => {
  return post<{ url: string }>(url)
}

export const dcoosUpdateSecretKey: Fetcher<any, { url: string; body: any }> = ({ url, body }) => {
  return post<any>(url, { body })
}