...
export const fetchDatasetDetail: Fetcher<DataSet, string> = (datasetId: string) => {
  ...
  return get<DataSet>(`/datasets/${datasetId}?tenant_id=${tenant_id || ''}`)
}
...
export const updateDatasetSetting: Fetcher<DataSet, {
  ...
  const tenant_id = getQueryParams('tenant_id');
  return patch<DataSet>(`/datasets/${datasetId}`, { body: { ...body, tenant_id } })
}
...
export const fetchDatasets: Fetcher<DataSetListResponse, { url: string; params: { page: number; ids?: string[]; limit?: number; tenant_id?: string } }> = ({ url, params }) => {
...
export const deleteDataset: Fetcher<DataSet, { params: { datasetID: string, tenant_id: string | undefined } }> = ({ params: { datasetID, tenant_id } }) => {
  return del<DataSet>(`/datasets/${datasetID}?tenant_id=${tenant_id || ''}`)
}
...
export const fetchDocuments: Fetcher<DocumentListResponse, { datasetId: string; params: { keyword: string; page: number; limit: number; sort?: SortType } }> = ({ datasetId, params }) => {
  const tenant_id = getQueryParams('tenant_id');
  return get<DocumentListResponse>(`/datasets/${datasetId}/documents?tenant_id=${tenant_id || ''}`, { params })
}
...
export const fetchIndexingStatusBatch: Fetcher<IndexingStatusBatchResponse, BatchReq> = ({ datasetId, batchId }) => {
  const tenant_id = getQueryParams('tenant_id');
  return get<IndexingStatusBatchResponse>(`/datasets/${datasetId}/batch/${batchId}/indexing-status?tenant_id=${tenant_id || ''}`, {})
}
...
export const fetchSegments: Fetcher<SegmentsResponse, CommonDocReq & { params: SegmentsQuery }> = ({ datasetId, documentId, params }) => {
  const tenant_id = getQueryParams('tenant_id');
  return get<SegmentsResponse>(`/datasets/${datasetId}/documents/${documentId}/segments?tenant_id=${tenant_id || ''}`, { params })
}
...
// hit testing
export const hitTesting: Fetcher<HitTestingResponse, { datasetId: string; queryText: string; retrieval_model: RetrievalConfig, tenant_id?: string }> = ({ datasetId, queryText, retrieval_model, tenant_id }) => {
  return post<HitTestingResponse>(`/datasets/${datasetId}/hit-testing`, { body: { query: queryText, retrieval_model, tenant_id: tenant_id || '' } })
}
...
export const fetchFileIndexingEstimate: Fetcher<FileIndexingEstimateResponse, IndexingEstimateParams> = (body: IndexingEstimateParams) => {
  const tenant_id = getQueryParams('tenant_id');
  return post<FileIndexingEstimateResponse>('/datasets/indexing-estimate', { body: { ...body, tenant_id: tenant_id || '' } })
}
