...
export const useDocumentList = (payload: {
  ...
  return useQuery<DocumentListResponse>({
    ...
    queryFn: () => get<DocumentListResponse>(`/datasets/${datasetId}/documents?tenant_id=${tenant_id || ''}`, {
      params: query,
    }),
    refetchInterval,
  })
...
export const useDocumentDelete = () => {
  ...
  return useMutation({
    mutationFn: ({ datasetId, documentIds, documentId }: UpdateDocumentBatchParams) => {
      return del<CommonResponse>(`/datasets/${datasetId}/documents?tenant_id=${tenant_id || ''}&${toBatchDocumentsIdParams(documentId || documentIds!)}`)
    },
  })
}
