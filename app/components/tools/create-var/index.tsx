...
import EditModal from '@/app/components/app/configuration/config-var/config-modal'
import s from '@/app/components/app/configuration/config-var/style.module.css'
...
const ConfigVar: FC<IConfigVarProps> = (props) => {
  const { promptVariables, readonly, onPromptVariablesChange, varTitle, btnTitle, limit, tableType, radioValue = 1 } = props
  ...
  const updatePromptVariable = (key: string, updateKey: string, newValue: string | boolean, idx: number) => {
  ...
  const updatePromptVariableItem = (payload: InputVar) => {
    const newPromptVariables = produce(promptVariables, (draft: any) => {
  ...
  const handleAddVar = (type: string) => {
    ...
    let newPromptVariables = []
    if (limit === 1 && promptVariables.length !== 0)
      newPromptVariables = [...promptVariables]
    else
      newPromptVariables = [...promptVariables, newVar]
  ...
  useEffect(() => {
    const mineTbodyHtml = document.getElementsByClassName('mineTbody')[1]
    if (radioValue === 2) {
      mineTbodyHtml.classList.add('active')
    } else {
      mineTbodyHtml?.classList.remove('active')
    }
  }, [radioValue])
    ...
    <Panel
      ...
      title={
        <div className='flex items-center'>
          <div style={{ fontFamily: 'Source Han Sans-Medium', fontWeight: '500', fontSize: '14px' }} className='mr-1'>{varTitle}</div>
          {/* {!readonly && (
      ...
      headerRight={!readonly ? <SelectVarType onChange={handleAddVar} btnTitle={btnTitle} /> : null}
    >
      {/* {!hasVar && (
      ...
      <div className='rounded-lg border border-gray-200 bg-white overflow-x-auto'>
        <table className={`${s.table} min-w-[440px] w-full max-w-full border-collapse border-0 rounded-lg text-sm`}>
          <thead className="border-b  border-gray-200 text-gray-500 text-xs font-medium">
            <tr className='uppercase bg-[#EDEEF0]'>
              <td>参数名称<span style={{ color: 'red' }}>*</span></td>
              <td>类型<span style={{ color: 'red' }}>*</span></td>
              <td>{'参数描述'}<span style={{ color: 'red' }}>*</span></td>
              {tableType === 'in' ? <td>默认值</td> : null}
              {tableType === 'in' ? <td>传输方式<span style={{ color: 'red' }}>*</span></td> : null}
              {!readonly && (
                <>
                  {tableType === 'in' ? <td className='flex justify-center'>是否必填</td> : null}
                  <td>{t('appDebug.variableTable.action')}</td>
                </>
              )}
          ...
          <tbody className="text-gray-700 mineTbody">
            {promptVariables.map(({ key, name, value, desc, getType, defaultValue, type, required, config, icon, icon_background }, index) => (
              <tr key={index} className="h-9 leading-9">
                <td className="border-b border-gray-100 pl-3">
                  <div className='flex items-center space-x-1'>
                    <input
                      ...
                      value={key}
                      onChange={e => updatePromptKey(index, e.target.value)}
                      className="h-6 leading-6 block w-full rounded-md border-0 py-1.5 text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:bg-[#F3F5F9] focus:ring-gray-200"
                    />
                  </div>
                </td>
                <td className="py-1 border-b border-gray-100">
                  <div className='h-6 leading-6 text-[13px] text-gray-700'>
                    <SelectedStyled size={'small'} bordered={false} className='custom-select focus:bg-[#F3F5F9] focus:outline-none' value={type} id="requestBodyType" onChange={e => updatePromptVariable(key, 'type', e, index)}>
                      <option value="string">String</option>
                      <option value="number">Number</option>
                      {/* {key === 'apiData' ? <option value="object">Object</option> : null} */}
                    </SelectedStyled>
                  </div>
                </td>
                <td className="border-b border-gray-100 pl-3">
                  <input
                    ...
                    placeholder="请输入参数描述"
                    value={desc}
                    onChange={e => updatePromptVariable(key, 'desc', e.target.value, index)}
                    // maxLength={getMaxVarNameLength(name)}
                    className="h-6 leading-6 block w-full rounded-md focus:bg-[#F3F5F9] border-0 py-1.5 text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                  />
                </td>
                {
                  tableType === 'in' &&
                  <td style={{ borderBottom: '1px solid rgb(234, 236, 240)' }}>
                    {!readonly
                      ? (
                        <input
                          type='text'
                          value={value}
                          placeholder="请输入默认值"
                          disabled={key === 'apiData' ? true : false}
                          onChange={e => updatePromptVariable(key, 'value', e.target.value, index)}
                          className='h-6 leading-6 block w-full focus:bg-[#F3F5F9] rounded-md border-0 py-1.5 text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200'
                        />
                      )
                      : (<div></div>)}
                    {/* {tableType === 'params' ? <div className=''>111</div> : null} */}
                  </td>
                }

                {
                  tableType === 'in' &&
                  <td style={{ borderBottom: '1px solid rgb(234, 236, 240)' }}>
                    <SelectedStyled size={'small'} bordered={false} className=' focus:bg-[#F3F5F9]' value={getType || 'Header'} onChange={(e) => updatePromptVariable(key, 'getType', e, index)}>
                  ...
                  <>
                    {
                      ...
                      <td className='w-[84px] border-b border-gray-100'>
                        <div className='flex items-center justify-center h-full'>
                          <span
                            onClick={e => updatePromptVariable(key, 'required', !required, index)}
                          >
                            {
                              required ?
                                <img style={{ width: '100%', height: '100%' }} src={`/agent-platform-web/image/checkbox-selected.png`} />
                                :
                                <img style={{ width: '100%', height: '100%' }} src={`/agent-platform-web/image/checkbox-unselected.png`} />
                            }
                          </span>
                          {/* <StyledCheckbox defaultChecked={required} onChange={e => updatePromptVariable(key, 'required', e.target.value, index)} /> */}
                        </div>
                      </td>
                    }
                    <td className='w-20  border-b border-gray-100'>
                      <div className='flex h-full items-center space-x-1'>
                        ...
                        <div className=' p-1 rounded-md hover:bg-black/5 text-red-500 cursor-pointer' onClick={() => handleRemoveVar(index)} >
                          删除
                        </div>
                      </div>
                    </td>
