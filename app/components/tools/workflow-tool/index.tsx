...
const WorkflowToolAsModal: FC<Props> = ({
  ...
  const [pluginsParams, setPluginsParams] = useState('')
  ...
  const onConfirm = () => {
    ...
    setPluginsParams(JSON.stringify(requestParams))
  }

  const publishPlugin = (res: any) => {
    const requestParams = {
      name,
      description,
      icon: emoji,
    ...
    <>
      ...
      {showReleaseModal && (
        <ReleaseModalMerge
          ...
          release={showReleaseModal}
          appId={payload.workflow_app_id}
          // status={getQueryParams('status')}
          tabClick={getQueryParams('tabClick')}
          done={(res) => publishPlugin(res)}
          payload={payload}
          onCreate={onCreate}
          currentTab={'workflow'}
        />
