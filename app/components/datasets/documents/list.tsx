...
export const OperationAction: FC<{
  ...
  scene?: 'list' | 'detail'
  className?: string
}> = ({ embeddingAvailable, shieldButton, datasetId, detail, onUpdate, scene = 'list', className = '' }) => {
  ...
  const postMessage = (dataType: string, param?: object) => {
    window.parent.postMessage({ type: 'newDataSet', value: dataType, ...param }, '*');
  }
          ...
          >
            <button
              ...
              onClick={() => {
                ...
                postMessage('settings', { id: datasetId, docId: detail.id })
                // router.push(`/datasets/${datasetId}/documents/${detail.id}/settings?tenant_id=${tenant_id}`)
              }}>
          ...
          <tbody className="text-text-secondary">
            {localDocs.map((doc, index) => {
              ...
              return <tr
                ...
                onClick={() => {
                  postMessage("documentsMark", { id: datasetId, docId: doc.id });
