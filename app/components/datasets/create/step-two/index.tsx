...
const StepTwo = ({
  ...
  const updatePreview = () => {
    const { rules } = getProcessRule() || {};
    const { chunk_overlap, max_tokens, separator } = rules?.segmentation

    console.log(chunk_overlap, max_tokens, separator, chunk_overlap, max_tokens, separator)
    if (chunk_overlap === undefined || !max_tokens || separator === '') {
      message.info('分段标识符、分段最大长度、分段重叠长度不能为空');
      return;
    }
    if (max_tokens < 50 || max_tokens > 2000) {
      message.info('分段最大长度应为50~2000')
      return;
    }
    if (chunk_overlap > max_tokens) {
      message.info('分段重叠长度需小于分段最大长度')
      return;
    }
          ...
          {currentDocForm === ChunkingMode.qa && estimate?.qa_preview && (
            estimate?.qa_preview.map((item, index) => (
              <ChunkContainer
                key={item.question}
                label={`Chunk-${index + 1}`}
                characterCount={item?.num_tokens || (item.question.length + item.answer.length)}
              >
                <QAPreview qa={item} />
              </ChunkContainer>
            ))
          ...
          {currentDocForm === ChunkingMode.text && estimate?.preview && (
            estimate?.preview.map((item, index) => (
              <ChunkContainer
                key={item?.content}
                label={`Chunk-${index + 1}`}
                characterCount={item?.num_tokens || item?.content?.length}
              >
                {item?.content}
              </ChunkContainer>
            ))
