...
const ConfigPrompt: FC<Props> = ({
  ...
  useEffect(() => {
    const newPrompt = produce(payload as PromptItem[], (draft) => {
      if (provider === PromptRole.tool_chain && Array.isArray(payload) && payload.length > 0 && payload[0].role === PromptRole.system) {
        draft.shift();
      } else if (provider !== PromptRole.tool_chain && Array.isArray(payload)) {
        if (payload.length === 0 || payload[0].role !== PromptRole.system) {
          draft.unshift({ role: PromptRole.system, text: '' });
        }
      }
    });
    onChange(newPrompt);
  ...
  const handleAddPrompt = useCallback(() => {
    const newPrompt = produce(payload as PromptItem[], (draft) => {
      if (draft.length === 0 && provider !== PromptRole.tool_chain) {
        draft.push({ role: PromptRole.system, text: '' })
        return
      }
      const isLastItemUser = draft.length === 0 ? false : draft[draft.length - 1].role === PromptRole.user
                    ...
                    return (
                      <div key={item.id || index} className='relative group'>
                        ...
                        <ConfigPromptItem
                          ...
                          canRemove={provider === PromptRole.tool_chain ? true : payload.length > 1 && !(index === 0 && item.role === PromptRole.system)}
                          readOnly={readOnly}
                          id={item.id!}
