...
const useConfig = (id: string, payload: ToolNodeType) => {
  ...
  */
  ...
  const currentTools = (() => {
    switch (provider_type) {
      case CollectionType.builtIn:
        return (buildInTools && buildInTools.length > 0) ? buildInTools : newBuildInTools;
      case CollectionType.custom:
        return (customTools && customTools.length > 0) ? customTools : newApiTools;
      case CollectionType.workflow:
        return (workflowTools && workflowTools.length > 0) ? workflowTools : newWorkflowTools;
      default:
        return [];
    }
  })();
  const currCollection = currentTools.find((item: any) => item.id === provider_id)
  // Auth
  ...
  const currTool = currCollection?.tools.find((tool: any) => tool.name === tool_name)
