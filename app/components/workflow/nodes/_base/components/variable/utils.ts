...
import { type LLMNodeType, Type } from '../../../llm/types'

import type { KnowledgeRetrievalNodeType } from '../../../knowledge-retrieval/types'
import type { AgentNodeType } from '../../../agent-node/types'
import type { RAGNodeType } from '../../../rag-node/types'
...
import type { ParameterPasserNodeType } from '../../../parameter-passer/types'
...
import {
  AGENT_NODE_OUTPUT_STRUCT,
  RAG_NODE_OUTPUT_STRUCT,
  ANSWER,
...
const formatItem = (
  ...
  switch (data.type) {
    ...
    case BlockEnum.HttpRequest: {
      ...
      res.vars = HTTP_REQUEST_OUTPUT_STRUCT.concat(Object.keys(outputs || {}).map((key) => {
        return {
          variable: key,
          type: outputs[key].type,
        }
    ...
    case BlockEnum.ParameterPasser: {
      const {
        outputs,
      } = data as ParameterPasserNodeType
      res.vars = Object.keys(outputs).map((key) => {
        return {
          variable: key,
          type: outputs[key].type,
    ...
    case BlockEnum.AgentNode: {
      res.vars = AGENT_NODE_OUTPUT_STRUCT
      break
    }
    case BlockEnum.RAGNode: {
      res.vars = RAG_NODE_OUTPUT_STRUCT
      break
    }
...
export const getNodeUsedVars = (node: Node): ValueSelector[] => {
  ...
  switch (type) {
    ...
    case BlockEnum.Answer: {
      res = (data as AnswerNodeType).variables?.map((v: any) => {
        return v.value_selector
      })
      break
    }
    case BlockEnum.AgentNode: {
      res = (data as AgentNodeType).variables?.map((v: any) => {
        return v.value_selector
      })
      break
    }
    case BlockEnum.RAGNode: {
      res = (data as RAGNodeType).variables?.map((v: any) => {
        return v.value_selector
      })
      break
    }
...
export const updateNodeVars = (oldNode: Node, oldVarSelector: ValueSelector, newVarSelector: ValueSelector): Node => {
  const newNode = produce(oldNode, (draft: any) => {
    ...
    switch (type) {
      ...
      case BlockEnum.ParameterPasser: {
        const payload = data as ParameterPasserNodeType
        if (payload.variables) {
          payload.variables = payload.variables.map((v: any) => {
            if (v.value_selector.join('.') === oldVarSelector.join('.'))
              v.value_selector = newVarSelector
...
export const getNodeOutputVars = (node: Node, isChatMode: boolean): ValueSelector[] => {
  ...
  switch (type) {
    ...
    case BlockEnum.AgentNode: {
      varsToValueSelectorList(AGENT_NODE_OUTPUT_STRUCT, [id], res)
      break
    }

    case BlockEnum.RAGNode: {
      varsToValueSelectorList(RAG_NODE_OUTPUT_STRUCT, [id], res)
      break
    }
