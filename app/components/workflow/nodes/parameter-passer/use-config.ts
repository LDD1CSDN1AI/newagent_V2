...
import type { VarGroupItem, ParameterPasserNodeType, OutputVar } from './types'
...
const useConfig = (id: string, payload: ParameterPasserNodeType) => {
  const { nodesReadOnly: readOnly } = useNodesReadOnly()
  const { handleOutVarRenameChange, isVarUsedInNodes, removeUsedVarInNodes } = useWorkflow()

  const { inputs, setInputs } = useNodeCrud<ParameterPasserNodeType>(id, payload)
  ...
  useEffect(() => {
    if (Object.keys(inputs.outputs).length === 0) {
      setInputs({ ...inputs, outputs: { result: { type: 'string', children: null } } })
    }
  ...
  } = useOutputVarList<ParameterPasserNodeType>({
    id,
    inputs,
    setInputs,
    outputKeyOrders,
