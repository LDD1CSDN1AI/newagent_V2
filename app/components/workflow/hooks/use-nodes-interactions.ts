...
import { getNodeCustomTypeByNodeDataType } from '../utils'
...
export const useNodesInteractions = () => {
  ...
  const handleNodeEnter = useCallback<NodeMouseHandler>((_, node) => {
    ...
    if (connectingNodePayload) {
      ...
      if (sameLevel) {
        ...
        const newNodes = produce(nodes, (draft) => {
          draft.forEach((n) => {
            if (n.id === node.id && fromType === 'source' && (node.data.type === BlockEnum.VariableAssigner || node.data.type === BlockEnum.VariableAggregator || node.data.type === BlockEnum.ParameterPasser)) {
              if (!node.data.advanced_settings?.group_enabled)
                n.data._isEntering = true
            }
            if (n.id === node.id && fromType === 'target' && (connectingNode.data.type === BlockEnum.VariableAssigner || connectingNode.data.type === BlockEnum.VariableAggregator || connectingNode.data.type === BlockEnum.ParameterPasser) && node.data.type !== BlockEnum.IfElse && node.data.type !== BlockEnum.QuestionClassifier)
              n.data._isEntering = true
          })
        })
        setNodes(newNodes)
  ...
  const handleNodeConnectStart = useCallback<OnConnectStart>((_, { nodeId, handleType, handleId }) => {
    ...
    if (nodeId && handleType) {
      ...
      if (node.data.type === BlockEnum.VariableAggregator || node.data.type === BlockEnum.VariableAssigner || node.data.type === BlockEnum.ParameterPasser) {
  ...
  const handleNodeConnectEnd = useCallback<OnConnectEnd>((e: any) => {
    ...
    if (connectingNodePayload && enteringNodePayload) {
      ...
      if (fromHandleType === 'source' && (toNode.data.type === BlockEnum.VariableAssigner || toNode.data.type === BlockEnum.VariableAggregator || toNode.data.type === BlockEnum.ParameterPasser)) {
