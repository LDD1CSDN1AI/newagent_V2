...
import { fetchInstalledAppList as doFetchInstalledAppList, fetchInstalledShareAppList, uninstallApp, updatePinStatus } from '@/service/explore'
...
const SideBar: FC<IExploreSideBarProps> = ({
  ...
  const fetchInstalledAppList = async (id?: string) => {
    const isShare = decodeURIComponent(getChineseParamFallback(window.location.href.toString(), 'isShare') || '');
    const { installed_apps }: any = isShare && isShare !== 'undefined' ? await fetchInstalledShareAppList(id) : await doFetchInstalledAppList(id);
    setInstalledApps(installed_apps)
  }
  const [showConfirm, setShowConfirm] = useState(false)
  const [currId, setCurrId] = useState('')
  const [items, setItemList] = useState<any>([])
  ...
  const tanentId = getQueryParams('tanent_id')

  useEffect(() => {
    fetchInstalledAppList(tanentId)
  }, [])
  ...
  useEffect(() => {
    ...
    if (installedApps && installedApps.length > 0) {
      ...
      installedApps.forEach((item) => {
        arr.push({
          label: (<div onClick={() => itemClick(item)}>{item.app.name}</div>),
          key: item.id,
        })
        if (item.id === id)
          setItemTitle(item.app.name)
      })
      setItemList(arr)
    }
  }, [installedApps])
  ...
  return (
    <div className='w-full h-[50px] flex' style={{ lineHeight: '50px' }}>
      <div onClick={() => {
        pushBackHistory()
      }}>
        <Image style={{ display: 'inline-block', margin: '8px 10px 10px 10px', cursor: 'pointer' }} src={back} alt='img' width={16} height={16} className='mx-auto mt-[6px]' />
      </div>
      <div style={{ cursor: 'pointer' }} onClick={() => { pushBackHistory() }}>返回</div>
      {/* {
        items.length > 0 ?
