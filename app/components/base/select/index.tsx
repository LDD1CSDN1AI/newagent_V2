  ...
  return (
    ...
    >
      <div className={`relative h-9 ${wrapperClassName}`}>
        <Listbox.Button className={`w-full h-full rounded-lg border-0 bg-gray-300 py-1.5 pl-3 pr-10 sm:text-sm sm:leading-6 focus-visible:outline-none focus-visible:bg-gray-200 group-hover:bg-gray-200 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} ${className}`}>
...
const ElementSelect: FC<ElementSelectProps> = ({
  className,
  items = defaultItems,
  defaultValue = 1,
  disabled = false,
      ...
      <div className={classNames('relative')}>
        <div className='text-gray-800'>
          {allowSearch
            ...
            } className={`flex items-center h-9 w-full py-1.5 pl-3 pr-10 sm:leading-6 focus-visible:outline-none focus-visible:bg-gray-100 group-hover:bg-gray-100 ${bgClassName}`}>
              {/* <div className='w-0 grow text-left truncate text-slate-500' title={selectedItem?.name ?? localPlaceholder}>{selectedItem?.name ?? localPlaceholder}</div> */}
              <div className='w-0 grow text-left truncate text-gray-500 font-normal' title={selectedItem?.name ?? localPlaceholder}>{selectedItem?.name ?? localPlaceholder}</div>
            </Combobox.Button>}
          <Combobox.Button className="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none group-hover:bg-gray-200" onClick={
            () => {
              if (!disabled)
...
export { SimpleSelect, PortalSelect, ElementSelect }
