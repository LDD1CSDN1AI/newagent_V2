...
const CreateFromDSLModal = ({ show, onSuccess, onClose, tenant_id, tabClick, fromType }: CreateFromDSLModalProps) => {
  ...
  const onCreate: MouseEventHandler = async () => {
    ...
    if (!currentFile)
      return
    try {
      let param = tenant_id ? { data: fileContent || '', tenant_id } : { data: fileContent || '' };
      ...
      getRedirection(isCurrentWorkspaceEditor, { ...app, appId: app.id, tabClick, fromType, tenant_id }, push)
    }
    catch (e) {
