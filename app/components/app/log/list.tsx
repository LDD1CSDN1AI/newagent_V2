...
function DetailPanel<T extends ChatConversationFullDetailResponse | CompletionConversationFullDetailResponse>({ detail, onFeedback }: IDetailPanel<T>) {
  ...
  const isAdvanced = appDetail?.mode === 'metabolic'
...
const ConversationList: FC<IConversationList> = ({ logs, appDetail, onRefresh }) => {
  ...
  const isworkflow = appDetail.mode === 'advanced-chat' // Whether the app is a workflow app
      ...
      <table className={`w-full min-w-[440px] border-collapse border-0 text-sm mt-3 ${s.logTable}`}>
        <thead className="h-8 leading-8 border-b border-gray-200 text-gray-500 font-bold">
          <tr>
            ...
            {isworkflow && <td className='whitespace-nowrap bg-background-section-burn py-1.5 pl-3'>{t('appLog.table.header.status')}</td>}
