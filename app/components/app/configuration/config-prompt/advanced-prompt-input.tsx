...
const Prompt: FC<ISimplePromptInput> = ({
  ...
  // 创建一个防抖的触发自动保存函数
  const triggerAutoSave = React.useMemo(() => debounce(() => {
    const autoSaveEvent = new Event('auto-save')
      ...
      <div>
        ...
        >
          <PromptEditor
            ...
            placeholder="请用自然语言描述大模型的功能及工作流程，如需其他变量可在下方进行添加。举例：你是一名{职业}，具备{功能}，根据以下信息进行规划：{KEY1}，{KEY2}。用{}()定义变量。"
            contextBlock={{
              show: false,
