...
export const useProviderCredentialsAndLoadBalancing = (
  ...
  const { data: predefinedFormSchemasValue, mutate: mutatePredefined } = useSWR(
    (configurationMethod === ConfigurationMethodEnum.predefinedModel && configured)
      ? `/workspaces/current/model-providers/${provider}/credentials?tenant_id=${tenantId}`
      : null,
    fetchModelProviderCredentials,
  )
  const { data: customFormSchemasValue, mutate: mutateCustomized } = useSWR(
    (configurationMethod === ConfigurationMethodEnum.customizableModel && currentCustomConfigurationModelFixedFields)
      ? `/workspaces/current/model-providers/${provider}/models/credentials?model=${currentCustomConfigurationModelFixedFields?.__model_name}&model_type=${currentCustomConfigurationModelFixedFields?.__model_type}&tenant_id=${tenantId}`
      : null,
    fetchModelProviderCredentials,
  )
...
export const useModelProviders = () => {
  const { data: providersData, mutate, isLoading } = useSWR(`/workspaces/current/model-providers?tenant_id=${tenantId}`, fetchModelProviders)
...
export const useUpdateModelProviders = () => {
  ...
  const updateModelProviders = useCallback(() => {
    mutate(`/workspaces/current/model-providers?tenant_id=${tenantId}`)
  }, [mutate])
