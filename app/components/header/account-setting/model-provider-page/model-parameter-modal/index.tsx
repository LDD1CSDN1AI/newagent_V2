...
import { getTenantIdFromUrl } from '@/utils/getUrlParams'
...
const ModelParameterModal: FC<ModelParameterModalProps> = ({
  ...
  const { data: parameterRulesData, isLoading } = useSWR((provider && modelId) ? `/workspaces/current/model-providers/${provider}/models/parameter-rules?model=${modelId}&tenant_id=${tenantId}` : null, fetchModelParameterRules)
  const {
    currentProvider,
    currentModel,
  ...
  useEffect(() => {
    setModelName(currentModel?.label.zh_Hans)
  }, [currentModel])
  ...
  const handleChangeModel = ({ provider, model }: DefaultModel) => {
    ...
    setModelName(targetModelItem?.label.zh_Hans)
    setModel({
      modelId: model,
      provider,
  ...
  return (
    ...
    >
      <div className='relative'>
        ...
        <PortalToFollowElemContent className={cn(portalToFollowElemContentClassName, 'z-[999]')}>
          <div className={cn(popupClassName, 'w-[496px] rounded-xl border border-gray-100 bg-white shadow-xl')}>
            ...
            {!hideDebugWithMultipleModel && (
              <div className='flex items-center justify-between px-6 h-[50px] bg-gray-50 border-t border-t-gray-100 text-xs font-medium text-gray-400 rounded-b-xl'>
                {
                  debugWithMultipleModel
                    ? t('appDebug.debugAsSingleModel')
                    : t('appDebug.debugAsMultipleModel')
