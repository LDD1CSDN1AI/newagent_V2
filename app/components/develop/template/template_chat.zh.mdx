...
<Row>
  <Col>
    ...
    - `event: agent_message` Agent模式下返回文本块事件，即：在Agent模式下，文章的文本以分块的方式输出（仅Agent模式下使用）
      ...
      - `answer` (string) LLM 返回文本块内容
      - `created_at` (int) 创建时间戳，如：1705395332
    - `event: agent_thought` Agent模式下有关Agent思考步骤的相关内容，涉及到插件调用（仅Agent模式下使用）
      ...
      - `observation` (string) 插件调用的返回结果
      - `tool` (string) 使用的插件列表，以 ; 分割多个插件
      - `tool_input` (string) 插件的输入，JSON格式的字符串(object)。如：`{"dalle3": {"prompt": "a cute cat"}}`
      - `created_at` (int) 创建时间戳，如：1705395332
            ...
            "message_files": [
                {
                    ...
                    "type": "image",
                    "url": "http://172.27.221.54:5001/files/tools/976990d2-5294-47e6-8f14-7356ba9d2d76.png?timestamp=1705988524&nonce=55df3f9f7311a9acd91bf074cd524092&sign=z43nMSO1L2HBvoqADLkRxr7Biz0fkjeDstnJiCK1zh8=",
