...
const DatasetCard = ({
  ...
  const onConfirmDelete = useCallback(async () => {
    try {
      ...
      await deleteDataset({ params: { datasetID: dataset.id, tenant_id } })
    ...
    return (
      <div className="relative w-full py-1" onMouseLeave={onMouseLeave}>
        <div className='mx-1 flex h-8 cursor-pointer items-center gap-2 rounded-lg px-3 py-[6px] hover:bg-state-base-hover' onClick={onClickRename}>
          <span className='text-sm text-text-secondary'>{t('common.operation.rename')}</span>
        </div>
        {props.showDelete && (
          <>
      ...
      <div
        ...
        onClick={(e) => {
          ...
          isExternalProvider(dataset.provider)
            ? postMessage('hitTesting', { id: dataset.id })
            : postMessage('documents', { id: dataset.id })
          // isExternalProvider(dataset.provider)
