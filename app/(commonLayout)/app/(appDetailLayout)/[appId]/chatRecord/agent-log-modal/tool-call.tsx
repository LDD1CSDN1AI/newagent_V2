...
import { getAgentWokflowLog } from '@/service/apps';
...
const columns: TableColumnsType<DataType> = [
  { title: '名称/ID', dataIndex: 'title' },
  { title: 'input', dataIndex: 'query', render: (_, item) => JSON.stringify(item?.inputs || '') },
  { title: 'output', dataIndex: 'answer', render: (_, item) => JSON.stringify(item?.outputs || '') },
...
const ToolCallItem: FC<Props> = ({ toolCall, isLLM = false, isFinal, tokens, observation, finalAnswer }) => {
  ...
  const [isWorkflowLogModalOpen, setIsWorkflowLogModalOpen] = useState<boolean>(false)
  const [workflowLogData, setWorkflowLogData] = useState([])
  const getWorkflowLog = (toolName) => {
    getAgentWokflowLog({
      params: {
        tool_name: toolName
      }
      ...
      console.log('res', res)
      if (res.length >= 0) {
        setIsWorkflowLogModalOpen(true)
        setWorkflowLogData(res)
      }
    })
  }

  const handleCancel = () => {
    setIsWorkflowLogModalOpen(false)
    setWorkflowLogData([])
  }
    ...
    <div className={cn('py-1')}>
      <div className={cn('group transition-all bg-white border border-gray-100 rounded-2xl shadow-xs hover:shadow-md')}>
        ...
        >
          ...
          )} title={toolName} onClick={() => !isLLM && getWorkflowLog(toolName)}>{toolName}</div>
