...
const AppDetailLayout: FC<IAppDetailLayoutProps> = (props) => {
    ...
    const navs = [
      ...(isCurrentWorkspaceEditor
        ? [{
          ...
          href: `/app/${appId}/${(mode === 'workflow' || mode === 'metabolic' || mode === 'advanced-chat') ? 'workflow' : 'configuration'}`,
          // href: `/app/${appId}/${(mode === 'workflow' || mode === 'metabolic') ? 'workflow' : 'configuration'}`,
  ...
  useEffect(() => {
    if (appDetail) {
      ...
      // if ((appDetail.mode === 'metabolic' || appDetail.mode === 'workflow') && (pathname).endsWith('workflow'))
  ...
  useEffect(() => {
    ...
    fetchAppDetail({ url: '/apps', id: appId }).then((res) => {
      // redirections
      if ((res.mode === 'workflow' || res.mode === 'metabolic' || res.mode === 'advanced-chat') && (pathname).endsWith('configuration')) {
        router.replace(`/app/${appId}/workflow`)
      }
      else if ((res.mode !== 'workflow' && res.mode !== 'metabolic' && res.mode !== 'advanced-chat') && (pathname).endsWith('workflow')) {
