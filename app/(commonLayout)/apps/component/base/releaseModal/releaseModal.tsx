...
import { pluginPublishedToPersonalSpace, publishedPersonal, agentFaBu, pluginReleaseAudit, workspaceAgentPublicAudit, workspacePublicAudit, checkPluginName, sharePluginReleaseAudit, shareReleaseAudit, publishApi } from '@/service/apps'
...
const CreateModal: React.FC<Props> = (props) => {
  const { listData, directRelease, tabClick: tabClickValue, appId = props.appId ?? getQueryParams('app_id'), fromType: fromTypeValue, release, tenant_id: tenant_id_value, onClose, done, data = {}, payload = {}, parameter = [], descriptions = '', isShowPublished = false, currentTab = '', onCreate, mutate } = props
  ...
  const [parentRadioValue, setParentRadioValue] = useState<any>(isShowPublished ? 2 : 1)
  ...
  const selectSpace = (data: any) => {
    const e = data || parentRadioValue;
    if (e + '' === '1' || e + '' === '2') {
  ...
  const PublishedSuccessfully = async () => {
    ...
    const target = listData.filter(item => item.tenant_id === releaseTypeId)

    const getTip = (type: string) => {
      let tip = '';
      if (type === '成功') {
        if (target?.[0]?.display_name === '发布至广场') {
          tip = '已至超管审核，通过后即可发布成功'
        } else {
          tip = target?.[0]?.type === 'project' ? `已发布至项目空间：${target?.[0]?.tenant_name}` : target?.[0]?.display_name + type;
        }
      } else if (type === '失败') {
        tip = '发布失败';
      }
    ...
    if (!appId) {
      ...
      if (tabClick === '2') {
        if (releaseType === 'project') {
          ...
          if (res.detail == '流程已创建，等待项目管理员审批') {
            message.success(res.detail)
          } else if (res.result == 'success') {
            message.success(getTip('成功'))
            mutate?.()
          } else {
            message.error(getTip('失败'))
          }
          setRelease(false)
          onClose?.()
          return
        ...
        if (releaseType === 'share') {
          ...
          // 插件发布到共享广场
          const res: any = await sharePluginReleaseAudit({
            appId,
            body: param
          })
        ...
        if (releaseType === 'default') {
          ...
          // agent 广场
          const res: any = await shareReleaseAudit({
            appId,
            body: param
          })
          if (res.detail == '流程已创建，等待项目管理员审批') {
  ...
  const directReleaseFunc = async () => {
    const params = {
      tenant_id: fromType === '项目空间' ? tenant_id : '',
      app_id: appId,
      desc: tempValue1
    }

    const res: any = await publishApi(params)
  ...
  console.log(release1, '--------------------------------release1')
  return (
    <Modal
      title={'发布应用'}
      open={release1}
      onOk={parentRadioValue + '' === '3' ? directReleaseFunc : PublishedSuccessfully}
        ...
        <Form layout='vertical'>
          {!isShowPublished &&
            <>
              ...
              {
                releaseType === "default" || tabClick === '1' || tabClick === '3' ?
              ...
              {
                (currentTab === 'workflow' || tabClick + '' === '1' ? <Form.Item label='请求方式' name='name' rules={[{ required: true, message: '11111111111' }]}>
                  <Radio.Group onChange={(e: any) => { setParentRadioValue(e.target.value); selectSpace(e.target.value) }} defaultValue={parentRadioValue}>
                    <Radio value={1}>
                      {
                        (tabClick === '2' || tabClick === '4' || tabClick === '5') ?
                          '发布仅体验'
                          :
                          '发布到广场'
                      }
                    ...
                    {
                      ((releaseType !== 'public' && releaseType !== 'share' && currentTab === 'workflow') || tabClick === '3') &&
                      <Radio value={2}>发布可被Agent调用</Radio>
                    }
                    {
                      (tabClick === '2' || tabClick === '4' || tabClick === '5') ? '' :
                        <Radio value={3}>发布成API</Radio>
                    }
                  </Radio.Group>
                </Form.Item> : null)
              }
